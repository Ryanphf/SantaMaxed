...
(function() {
  ...
  koala.makeCircles = function(selector, colorData, onEvent) {
   ...
   window.splitableCircleAt = function(pos) {
    ...
    window.findAndSplit = function(startPoint, endPoint) {
    ...
    function onMouseMove() {
      ...
      if (prevMousePosition) {
        window.findAndSplit(prevMousePosition, mousePosition);
    ...
    function onTouchMove() {
      ...
      for (var touchIndex = 0; touchIndex < touchPositions.length; touchIndex++) {
        ...
        if (prevTouchPosition) {
          window.findAndSplit(prevTouchPosition, touchPosition);
        }
  ...
  var preRandom = Math.random()*510;
    koala.automation = function()  {
    
    var random = Math.random() * 510;
    ...
    .enter()
    .append('circle')
    .attr('cx', (d, i) => i * 50 + 50) // Set the x-coordinate of the center
    .attr('cy', 200) // Set the y-coordinate of the center
    .attr('r', d => d) // Set the radius
    .attr('fill', 'steelblue'); // Set the fill color
      
    var quad = splitableCircleAt(random);
    if(quad.isSplitable && intervalLength != NaN) {
      window.findAndSplit(-preRandom,random);
    }  
     preRandom = random;
